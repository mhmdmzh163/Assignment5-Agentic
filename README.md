...
2. Nominatim (Geocoding & POI server)
   ...
   ```
   a. Nominatim’s usage policy requires that clients:
   ...
   b. In this project, these details are configured via GeocodingServerParams in map_servers/params.py:
     - base_url – the Nominatim endpoint.
     - user_agent – the custom User-Agent string.
     - timeout – HTTP timeout for requests.
   c. If you want to use this beyond a small demo, consider:
