...
3. **OSRM (Routing server)**  
   - The routing server uses an OSRM HTTP endpoint, also configured in `map_servers/params.py`
     via `RoutingServerParams`:
   ...
   - In this project, `routing_server.py`:
     ...
     - Adds convenience fields:
       - `duration_min` for single routes.
       - `durations_min` for the distance matrix.
