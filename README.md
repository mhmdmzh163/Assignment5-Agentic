...
2. Nominatim (Geocoding & POI server)
   a. By default, the project uses the public Nominatim instance at:
     ```text
     https://nominatim.openstreetmap.org
     ```
   b. Nominatim’s usage policy requires that clients:
   ...
   c. In this project, these details are configured via GeocodingServerParams in map_servers/params.py:
       - base_url – the Nominatim endpoint.
       - user_agent – the custom User-Agent string.
       - timeout – HTTP timeout for requests.
   d. If you want to use this beyond a small demo, consider:
...
3. OSRM (Routing server)
   a. The routing server uses an OSRM HTTP endpoint, also configured in map_servers/params.py via RoutingServerParams:
   ...
   b. According to the OSRM HTTP API documentation:
       - distance values are returned in meters.
       - duration values are returned in seconds.
   c. In this project, routing_server.py:
   ...
   d. You can point RoutingServerParams.base_url to:
